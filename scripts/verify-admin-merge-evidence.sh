...
CLAUSE_FILTER='(contains("<!-- admin-merge-safety: '"$HEAD"' -->"))
  ...
  and ([ (. | split("\n"))[] | select(test("Attribution[^\\n[:cntrl:]\\x{2028}\\x{2029}]*tokens DROPPED")) ] | length > 0)
  and ([ (. | split("\n"))[] | select(test("Attribution[^\\n[:cntrl:]\\x{2028}\\x{2029}]*tokens DROPPED")) ] | all(test("^[ \\t]*Attribution — FAILED tokens DROPPED by the parser \\(not test ids, so NEVER in a failing set\\): PR=0 \\| main=(0|[1-9][0-9]*)\\.\\r?$")))
  and ((test("PR failing:\\s*0+\\s*\\|\\s*main failing:\\s*0+\\s*\\|") | not)
       or ([ (. | split("\n"))[] | select(test("Attribution[^\\n[:cntrl:]\\x{2028}\\x{2029}]*tokens DROPPED")) ] | all(test("^[ \\t]*Attribution — FAILED tokens DROPPED by the parser \\(not test ids, so NEVER in a failing set\\): PR=0 \\| main=0\\.\\r?$"))))
  and ((test("PR failing:\\s*0+\\s*\\|\\s*main failing:\\s*0+\\s*\\|") | not)
