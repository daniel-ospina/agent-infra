...
CLAUSE_FILTER='(contains("<!-- admin-merge-safety: '"$HEAD"' -->"))
  ...
  and ([ (. | split("\n"))[] | select(test("Attribution[^\\n]*tokens DROPPED")) ] | length > 0)
  and ([ (. | split("\n"))[] | select(test("Attribution[^\\n]*tokens DROPPED")) ] | all(test("^[ \\t]*Attribution — FAILED tokens DROPPED by the parser \\(not test ids, so NEVER in a failing set\\): PR=0 \\| main=(0|[1-9][0-9]*)\\.\\r?$")))
