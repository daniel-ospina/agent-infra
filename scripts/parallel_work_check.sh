...
# /tmp/parallel-check-token.json, PER-SESSION scoped to
# /tmp/parallel-check-token.<session-id>.json when run inside a pi session —
# PI_SESSION_ID is set; #378) is written ONLY on a CLEAR verdict —
...
#     per-session scoped to parallel-check-token.<PI_SESSION_ID>.json inside
#     a pi session, #378)
...
# #378 patch: the DEFAULT is now PER-SESSION scoped — pi injects PI_SESSION_ID
# (== ctx.sessionManager.getSessionId()) into bash-tool child envs, so a
# checker run inside a pi session writes /tmp/parallel-check-token.<sid>.json
# and the enforcer of THAT SAME session reads it (concurrent sessions no
# longer share/clobber one machine-global file). This wrapper MUST derive the
# identical path for its error-branch unlink (a watchdog SIGKILL skips
# python's own cleanup). Sanitization mirrors the python side (utf-8 bytes)
# and the enforcer (Node Buffer) BYTE-wise via `tr -c` — every byte outside
# [A-Za-z0-9._-] becomes '_', so the three implementations can never drift
# (session ids are ASCII UUIDs in practice; byte-wise keeps parity even then).
# No PI_SESSION_ID (operator shell run) or an explicit PARALLEL_CHECK_TOKEN_FILE
# → the legacy unscoped path / verbatim override, matching python + enforcer.
# LC_ALL=C pins `tr` to BYTE-wise semantics on every platform (a UTF-8-locale
# BSD tr maps one multibyte char → one '_' while GNU/Linux tr and python map
# each byte → a divergent filename; ids are ASCII in practice, but the C-locale
# pin keeps the three implementations byte-identical even for exotic input).
# The id is whitespace-trimmed first (python `.strip()` / TS `.trim()` parity)
...
if [ -z "$token_file" ]; then
    ...
    if [ -n "$token_sid" ]; then
        ...
        token_file="/tmp/parallel-check-token.json"
    fi
fi
