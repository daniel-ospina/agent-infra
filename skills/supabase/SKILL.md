...
When writing queries or touching the database schema, these two habits prevent silent bugs:

- **Verify column existence before `.select()`.** Before adding any column to a `.select()` string, verify the column exists in the database:
  ...
  Never assume a column exists because it appears in a hand-written TypeScript type — types can drift from the real schema after migrations.

- **Regenerate types after every migration.** After every migration, run and commit the updated types:
  ...
  This keeps the generated types in sync so `tsc --noEmit` catches mapping drift before PRs are created. Commit the types file alongside the `.sql` migration file.



> Also covered by supabase skill (#4696).
## Supabase CLI
...
When `apply_migration_safe` creates an auto-PR that adds migration files to `origin/main`, those files arrive on the remote but NOT in your local working tree. If you also had `git stash` active, popping the stash and pulling will conflict:

```
error: The following untracked working tree files would be overwritten by merge:
  supabase/migrations/20260710000001_add_business_ratings.sql
```
...
# 2. Remove the conflicting untracked files (they exist on remote, not yet local)
rm -f supabase/migrations/<auto-created-file>.sql

# 3. Now pull + pop cleanly
git pull --rebase
git stash pop
```

The auto-PR migration file is already merged to main — deleting the local untracked copy just clears the path for the pull to bring it in properly.
