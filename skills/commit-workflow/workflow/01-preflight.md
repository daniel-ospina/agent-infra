...
On main/master + no ISSUE_NUMBER?
  → Ask: "What should I name the new branch?" then create it in a NEW WORKTREE —
    agent-infra → `bash scripts/checkout-hygiene/hub-worktree.sh feat/issue-{N}-{slug}`;
    other repos → using-git-worktrees skill. NEVER in the hub: in-place
    `git checkout -b` is BLOCKED (#626).
```
...
4. **Micro tier:** code sets satisfy the dispatch via VGATE's own [VGATE] verification dispatch; docs-only micro sets (VGATE content-shape exempt) dispatch a lightweight reviewer naming the diff and returning a verdict on it — the multi-agent code-review gate stays skipped per 03-code-review.md
