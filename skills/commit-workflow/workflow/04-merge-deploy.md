...
> review-enforcer merge gate now waits for the GraphQL reset window and retries
> (bounded by `REVIEW_GATE_RATE_LIMIT_MAX_WAIT_MS`, default 10 min) instead of
> failing mid-ceremony. If a ceremony still hits "GraphQL: API rate limit already
> exceeded", wait for the window or raise the cap — do NOT re-record the review.
