...
**Final code reviewer (after all tasks):** resolve the model with the second-model gate guard (#716) — `bash "$AGENT_INFRA_PATH/scripts/check-second-model.sh" --probe` (network; writes `RESOLVED=<provider/id>` for the first **solvent+reachable** candidate, or `DEGRADED`). **`--probe` is the resolution AND the dispatch authority.** `bash "$AGENT_INFRA_PATH/scripts/check-second-model.sh" --print` is an **offline hint for inspection only** (never the dispatch source): it opens no socket, so it cannot verify solvency and emits a *candidate* (with a loud stderr notice). Dispatch `model=<RESOLVED id>` from `--probe` (the liveness gate is the dispatch authority; `--print` is an offline hint, never the resolution). With `$SECOND_MODEL` set the probe certifies **only** that id — if it is not solvent+reachable (or declares no probe endpoint in `preference`) the probe is `DEGRADED` and never falls through to a config default. Never a hardcoded default. **Fail-closed DEGRADED (no silent fallback):** if `--print` returns `**DEGRADED` for a malformed/unusable authority, or the probe exits non-zero, do NOT dispatch a substitute — dispatching `deepseek-flash` (pi's built-in task-subagent default) or any build-equivalent model yields a same-build "independent" review (the exact #716 defect). The probe never emits the `**DEGRADED` token — it prints plain `DEGRADED` and exits 1 — so the governing trigger is the `--probe` exit code; `--print`'s `**DEGRADED` covers only a malformed/unusable authority and is never the resolution. Literal-matching only `**DEGRADED` never trips fail-closed here. There is NO recordable degraded marker — check (f) hard-fails both forms: `model=**DEGRADED` is rejected as a reserved value (before the independence field is read), and `independent=DEGRADED` is rejected by design. STOP and escalate to a human: the guarded-surface change cannot merge until an independent model is funded or the operator authorizes a bypass (#860). `[#476 hop-leg]` is orthogonal and stackable — a dispatch that lands on a failover hop is annotated `[SECOND-MODEL-GATE][#476 hop-leg]`, and a hop-leg run is never presented as the configured second model. This is the two-tier review pattern — Flash handles per-task reviews, the second model serves as the senior reviewer for the final pass across the entire implementation. It catches what cheaper per-task reviewers miss. Pricing/base decision: issue #284, superseded by #716.
