...
**(b)–(e) are escalation exits, not completions.** They do not satisfy the Exit Conditions above, and the loop must never be reported or handed off as clean or complete. (Where a skill labels its *zero-issue* exit "convergence" — e.g. `prototype-review` — that is a clean exit under (a), not this rule.) On a non-clean exit → **escalate** — to the orchestrator agent, or to a human wherever a skill requires one (the Auto-Continue pause conditions apply in addition). Document the remaining issues and post **the exact marker the skill's own exit table defines**, where it defines one (`code-review` cap → `⚠️ Auto-fix reached the 10-cycle safety cap — unresolved issues remain`; `test-review` cap → `⚠️ Test review capped at 10 cycles — N issues remain:`). Use the skill's own label verbatim — do not invent a cap label for a convergence or stall exit, and do not relabel an exit the skill itself names otherwise. Then follow the skill's own path for that exit:
...
- **Paths that log-and-proceed** (`epic-plan`, `test-review`, `test-writing` when only P1/P2 remain — **non-exhaustive**): continue **only** with the skill's own marker posted where it defines one, and the remaining issues — including any P0 — recorded in the artifact, exactly as the skill directs. A capped exit is never described as clean or complete.
...
- ❌ Self-declare "I addressed the feedback" as completion
  Only "NO ISSUES FOUND" from a fresh reviewer is a valid **clean-completion** signal. Convergence, stall, abort, and cap exits close the loop only with the remaining issues documented and escalated (see Hard Cap).
