...
function _worktreeDiscardBlockReason(
  ...
  return [
    ...
    `       git worktree remove --force "$D"   # ALWAYS clean. If its record must`,
    `       go too, remove THAT record — a bare \`git worktree prune\` deregisters`,
    `       every worktree whose dir is not stat-able right now, unrelated`,
    `       siblings included (#1141).`,
