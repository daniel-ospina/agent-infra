 ...
 * a 8× improvement on the 4h age backstop it pre-empts and still far inside it.
 *
...
export function heartbeatKillDecision(
  ...
  //    ⚠️ #928 update: "the age backstop below" is no longer the ONLY bound for
  //    that population. Clause 1b (`tool-dead`) closes it whenever the buffering
  //    tool has demonstrated CPU work and then gone FLAT for C — a bound of
  //    max(S, C) rather than L. Precise scope, because a reader will otherwise
  //    over-read this: 1b covers the DEADLOCKED-SILENT shape (a tool that burned
  //    CPU and then stopped), not every silent tool. The #928 incident's `grep`
  //    was silent but its CPU was ADVANCING (69m50s), so 1b is inert for it BY
  //    DESIGN — reclassifying a demonstrably working tool as dead is the false
  //    kill this whole item exists to avoid. The age backstop remains the bound
  //    for a CPU-advancing silent tool, for one that never burned CPU at all (an
  //    I/O-bound command), and for any kind outside the child's allowlist.
  ...
  //     another. Flat CPU has two causes — a deadlock, and a legitimate I/O
  //     block — and the second is common and healthy: a download, a slow query,
  //     a `git fetch` on a bad link all accrue ~no CPU for many minutes while
  //     making real progress, and (crucially) they are ALSO silent, so
  //     `tool_updates` is 0 for them too. Without this latch such a tool would
  //     be killed at max(S, C) where the previous bound was the 4h age
  //     backstop. Requiring demonstrated CPU work is exactly clause 1's shape —
  //     only ever kill a tool that has demonstrated it works, then stopped —
  //     applied to the CPU channel instead of the output channel.
  //
  //     What flat CPU still does NOT prove (residual, stated not hidden): a tool
  //     that burned CPU and is NOW blocked on I/O for longer than C is classified
  //     deadlocked and killed. C (30 min) is sized for that residual, and the
  //     kill headline says what the evidence was rather than asserting a cause
  //     the parent cannot prove. The reverse error — never firing on a real
...
export function spawnSubAgent(model: string, provider: string, subAgentEnv: Record<string, string | undefined>, args: string[], signal?: AbortSignal, record?: DispatchRecordContext): Promise<{ content: any[]; details: Record<string, unknown> } | undefined> {
  return new Promise((resolve) => {
    ...
    const hbThresholds = {
      ...
      // #928: silent-tool CPU-liveness bound (0 = clause disabled).
    ...
    const finalize = (code: number | null, settlePath: "close" | "exit") => {
      ...
      if (cls === "cut") {
        ...
        const markerAgeMs = hbCtx.state.lastMarkerAt > 0 ? Date.now() - hbCtx.state.lastMarkerAt : -1;
        const aliveSummary = `Alive state: toolsInFlight=${hbCtx.state.toolsInFlight} turnActive=${hbCtx.state.turnActive} streamAgeMs=${hbCtx.state.streamAgeMs} toolAgeMaxMs=${hbCtx.state.toolAgeMaxMs} toolCpuMs=${hbCtx.state.toolCpuMs} toolCpuStallMs=${hbCtx.state.toolCpuStallMs} toolCpuAdvanced=${hbCtx.state.toolCpuAdvanced} everSawRealActivity=${hbCtx.state.everSawRealActivity} lastMarkerAgeMs=${markerAgeMs} tickCount=${hbCtx.state.tickCount} markerCount=${hbCtx.state.markerCount} firstMarkerLagMs=${hbCtx.state.firstMarkerAt > 0 ? hbCtx.state.firstMarkerAt - startedAt : -1} firstTickLagMs=${hbCtx.state.firstTickAt > 0 ? hbCtx.state.firstTickAt - startedAt : -1} firstActivityLagMs=${hbCtx.state.firstActivityAt > 0 ? hbCtx.state.firstActivityAt - startedAt : -1} everSawMsg=${hbCtx.state.everSawMsg} everSawTool=${hbCtx.state.everSawTool} toolsMaxInFlight=${hbCtx.state.toolsMaxInFlight} trace=[${hbCtx.state.activityTrace.join(",")}] ${repoStateText()}`;
    ...
    const heartbeat = setInterval(async () => {
      ...
      const headlines: Record<string, string> = {
        ...
        "tool-dead": `⚠️ Sub-agent's in-flight tool has produced no output for ${Math.round(hbCtx.state.streamAgeMs / 1000)}s AND consumed no CPU for ${Math.round(hbCtx.state.toolCpuStallMs / 1000)}s (bound ${Math.round(hbThresholds.cpuStallMs / 1000)}s) — treated as deadlocked, not slow. Partial results below — parent should decide: accept, re-dispatch, or escalate.`,
