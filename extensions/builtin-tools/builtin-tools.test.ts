...
test("heartbeat timeout default is 30 min, env-overridable (#489)", () => {
  ok(source.includes("HEARTBEAT_TIMEOUT_MS = Number(process.env.TASK_HEARTBEAT_TIMEOUT_MS) || 1_800_000"), "heartbeat timeout should default to 30 min (1_800_000ms) and be env-overridable — raised from 660s because pi print-mode buffers output, so long tool-call sequences looked like silence and killed productive sub-agents (#489)");
});
