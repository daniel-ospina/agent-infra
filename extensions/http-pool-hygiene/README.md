...
| Env | Default | Meaning |
|---|---|---|
| `PI_HTTP_POOL_HYGIENE` | `1` | `0`/`false` disables the extension entirely (transport untouched) |
...
- The wrapper is re-asserted on `session_start`/`turn_start` if anything
  replaced `globalThis.fetch`. The replacement is **adopted as the wrapper's new
  base** (`rebase`) rather than discarded, so instrumentation another extension
  installed after us survives; a replacement that itself delegates back into
  `globalThis.fetch` cannot recurse (the re-entrant call passes through to the
  fetch below every wrapper layer). If pi rebuilt its dispatcher because
  `httpIdleTimeoutMs` changed at runtime, the re-assert rebuilds ours too
  (`reconfigure`) instead of pinning the value that was current at install —
  including the retry allowlist, so a provider added to `models.json` after
  install is not permanently un-retryable. It is
  not clobbered by later
  `configureHttpDispatcher()` calls: pi only re-installs its own fetch when it
  still sees the fetch *it* installed.
