...
4. **Decide the verdict, don't infer it.** GREEN means every group's newest attempt is `status == "completed"` AND its `conclusion` is one of `success`, `neutral`, `skipped`. NOT green: `failure`, `timed_out`, `action_required`, `cancelled`, `stale`. **Zero groups is NOT green** — a sha carrying no check-runs at all is UNVERIFIED, never green (older commits reachable from `main` carry none). **Any newest attempt still queued or in progress means NOT YET KNOWN.** Report UNVERIFIED and NOT YET KNOWN as their own answers; never default either to green, and never guess a verdict from a pending check — that is the same error as reading a rollup. Note the case where a job that runs on sibling commits is simply ABSENT here: absence is not green either.
