...
3. **Resolve the workflow before grouping.** A check-run's `name` is the JOB name, not the workflow's, and two workflows can publish the same job name — grouping on the name alone merges them, so the newest of one masks the other (measured: two `packaging-smoke` runs, one `cancelled`, the other `success`). Take the run id from `details_url`, resolve its workflow, and group by **(workflow, job name)**, newest attempt per group ordered by **`id`** — `started_at` is nullable (a queued attempt has none) and is a tiebreak only. Do not group by `check_suite.id`: every re-run gets its own suite, which would shatter one check into one group per attempt.
4. **Decide the verdict, don't infer it.** GREEN requires all three: at least one group exists; every group's newest attempt is `status == "completed"` with `conclusion` in `success`/`neutral`/`skipped`; and every context the branch requires is present among the groups (`gh api repos/<o>/<r>/branches/<branch>/protection --jq .required_status_checks.contexts`) — **a required context that is absent is UNVERIFIED, not green**, which is what makes "this job runs on sibling commits but not on this one" not green. NOT GREEN: `failure`, `timed_out`, `action_required`, `cancelled`, `stale`. **UNVERIFIED: a sha with no check-runs at all** — that is "nothing here to read", neither green nor a failure; for a sha pushed moments ago, re-read after the registration window before concluding anything. **NOT YET KNOWN: any newest attempt whose `status != "completed"`** (the in-flight spellings are `queued`, `in_progress`, `waiting`, `requested`, `pending`). Report UNVERIFIED and NOT YET KNOWN as their own answers — never default either to green.
...
**What this rule does NOT decide: mergeability. "Not green" is not "red".** `cancelled` and `stale` are not green, and neither is a failure of the code. What BLOCKS a merge is the merge rail, not this rule: `scripts/admin-merge.sh` groups checks by **`(app, name)`** and treats `{success, neutral, skipped, cancelled, stale}` as non-red. That conflation is stated there as an accepted residual — its key merges two workflows that share a job name, which is exactly what step 3 above separates. **So for "may I merge" read the rail; for "is the tree healthy" read this rule** — and never report a `cancelled` group as a main-health failure.
