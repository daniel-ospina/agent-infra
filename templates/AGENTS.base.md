...
4. **Decide the verdict, don't infer it.** Classify each group's NEWEST attempt, then read the whole:
   - **RED** — `failure`, `timed_out`, `action_required`, `startup_failure`.
   - **NOT GREEN, AND NOT RED** — `cancelled`, `stale`. A cancellation is not a broken tree; what blocks a merge is the rail, below.
   - **GREEN** — `success`, `neutral`, `skipped`.
   - **IN FLIGHT** — any `status != "completed"` (`queued`, `in_progress`, `waiting`, `requested`, `pending`).
   Verdict: **RED if any group is red; GREEN only if the surface is non-empty, every group is green, and none is in flight; NOT YET KNOWN while any group is in flight** — in-flight outranks everything, so report it rather than a verdict. Those three are the only answers to "is main green" — never default an unknown to green.
   **Read the surface for absence, too, because a missing check is not a green check.** Zero check-runs is **UNVERIFIED** — "nothing here to read", neither green nor red; for a sha pushed moments ago, re-read after the registration window rather than concluding anything. And when a check that runs on sibling commits is absent here, the surface is **INCOMPLETE**, which is not a green surface either.
