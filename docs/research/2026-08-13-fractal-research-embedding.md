...
|---|---|---|
| P1 | Artifacts over prompts — "asking nicely isn't enough" (juanchi.dev; Standing Questions; RPI) | Every research point emits a **fixed-structure persisted artifact** (`### Pattern Research` / `### Integration Docs` blocks — names are a live consumer contract, see Internal Finding 4) consumable by the next stage and checkable by existing verifier gates |
...
1. **issue-scoping:** re-add a dedicated external-research stage (Phase 1.5) for Standard+Complex — axis-driven matrix, fixed output contract (`### Pattern Research` + `### Integration Docs` blocks — **exact names, they are live consumer contracts** for executing-plans Step 1.5, writing-plans 03/04, proportional-gates), findings persisted to the issue comment + research brief; problem-verify gate checks artifact presence/structure.
