...
- **`toolsInFlight > 0` exemption (the #198 structural fix):** the first-message
  ...
  2h task-path tool-stall clause (`DEFAULT_TASK_TOOL_STALL_MS`, #783),
  never cut at M. (The exported `DEFAULT_TOOL_STALL_MS` stays 6h for
  `extensions/subagent/`; the task path was split off to 2h so the in-flight
  bound fires 4h before the 6h hard cap.) Pinned by test E13. Note (#279): with
...
- **Cap 3× → 15-min storm-time guarantee (watchdog):** at the cap, a first
  ...
  design* (uncapped scale would let hung providers linger indefinitely; the 2h
  task-path tool-stall clause remains the backstop for in-flight tools).
