...
- **The watchdog's load bands are INERT below load 8 on the fleet's 10-core
  hosts:** the absolute 8/16 bands are 0.8×/1.6× per core there, so every load
  under 8 — including the 6.6–7.3 range observed in #1074 — leaves the
  load-aware machinery doing nothing at all (the bound is 1x, i.e. the static
