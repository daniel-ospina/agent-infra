...
aboutObjects: agent-infra, issue-667, issue-640, issue-646, issue-666, issue-669, issue-675, issue-713, codeowners, pipeline-compliance
---
...
| `/scripts/check-skill-lint.test.mjs` | The #254 frontmatter-validator fixture-regression suite. As of #675 (`#666`) the #637 pin-lockstep guards `(h)`/`(i)`/`(j)` no longer live here — see the next row. |
| `/scripts/check-pi-pin-lockstep.mjs` | The home #675 moved the #637 pin-lockstep guards `(h)`/`(i)`/`(j)` into, together with the #666 wiring guard and workflow content lock — the file #667's target names (443 lines out of `check-skill-lint.test.mjs`, 3614 into the new file). |
| `/scripts/check-workflow-lock.mjs` | #675's byte-level content lock: hashes the three pin-gate workflow files against `workflow-lock.json`. Replaced modelling GitHub Actions execution semantics (the denylist four review rounds defeated). |
| `/scripts/workflow-lock.json` | The lock itself — the recorded hashes `check-workflow-lock.mjs` compares against. Editing a locked workflow without re-locking is RED. |
| `/scripts/workflow-yaml.mjs` | Dependency-free YAML-subset reader the wiring guard and the content lock both parse workflows with. A weakened reader is a weakened guard: not decoding YAML escapes (`\u002d` for `-`) was #675's P1 fail-open. |
...
| Path | Why not |
|---|---|
| `docs/**` (incl. `docs/plans/*`) | #667's open decision 3. `docs/plans/*` is the artifact `check-pipeline-compliance.sh` **reads** (check (d)), not the gate. Guarding it would name the owner on every plan-doc pattern — which, per §3, has no effect on review or merge in a single-writer repo — while it cannot weaken the gate, since the check logic lives in the guarded script. #667 non-goal: "Reviewing docs or content paths generally." |
| `extensions/review-enforcer/{package.json,package-lock.json}` and the same for `verification-gate` | Dependency manifests. The pin-lockstep guard `(h)` *reads* `extensions/*/package.json`; the guarded artifact is the guard (`check-pi-pin-lockstep.mjs`), not each manifest. Owner-reviewing every lockfile bump is noise without protection. |
...
| `.husky/pre-commit`, `templates/.husky/pre-commit` | Local hook, not the judge; it cannot weaken the required check. The merge-relevant checks it runs are mirrored in CI (`check-cost-config.sh` → `ci.yml` cost-config + `ci-main.yml`; `check-pi-config-extensions.sh` → `ci.yml` pi-config-extensions + `ci-main.yml`; the `.agent-infra-version` pin → `drift-check.yml`; `check-agents-materialized.sh` → `ci.yml` `drift-check` + a step in the required `pipeline-compliance` job, running `tests/materialize-agents/run.sh`, per #1027). One leg remains local-only — `check-staged-skill-frontmatter.mjs`. (The hook is also currently dead in agent-infra: husky is not installed, so `core.hooksPath` is unset — #672. The #1027 leg does exercise the hook script in CI, from the suite's staged fixture; it is not invoked as husky's pre-commit hook.) |
...
- **Does (as of #667):** records the path list and names the owner on each pattern. In this repo that has **no effect on PR review or merge today** — no review is requested and nothing is blocked. GitHub's two review/merge CODEOWNERS behaviours are auto-requesting a review and blocking a merge when code-owner review is required; both are void here (next bullet). There is no per-PR "annotation" behaviour — no review is posted and no comment is added. A *display* behaviour does exist once the file lands (GitHub documents seeing the code owner for a file, with a hover tooltip, plus a `codeowners/errors` REST endpoint), but it changes nothing about review or merge.
