...
Steps:
1. **01-preflight.md** — sweep: the export-annotation parenthetical ("review-enforcer proportional blocking", now ~L241) — rewritten to the legacy/vestigial annotation ("the extension never consults this env var for gate decisions; session_start reads it only to warn that it has no effect; #493 owns dropping the export"). Note: this edit touches the #493-owned annotation SENTENCE (pre-implementing #493 T2's re-label option — #493 coordination comment updated to match); the export COMMAND + marker-write lines (L244-245) and the VGATE-SHAPE-RULE fence (L338-345 post-fence-insertion) are untouched. Also sweep: the mechanism-separation sentence (final L252; written pre-#500 as L250 — anchors in this step list are the PRE-#500/pre-fence-insertion numbering per the risk register's content-first rule; match by unique text, not line numbers): reshape (the marker selects the MESSAGE, not allow/block — labeled-micro docs-only commits now block like unlabeled; VGATE shape-exemption unchanged); the Micro table Review-enforcer row (final L256; pre-#500 L254) → **BLOCK at 0 dispatches — ≥1 task dispatch required at EVERY tier (micro included since #485)**; the rationale paragraph (final L261; pre-#500 L259) (micro code sets satisfy via VGATE's own dispatch; the residual case the uniform block closes is docs-only micro); the extension-gates intro (final L292; pre-#500 L290) (uniform ≥1-dispatch block; marker read selects the micro message); Review Enforcer Gate prose + one how-to-satisfy line (final L296/L304-308; pre-#500 L293-294/L298-306) documenting the micro block message.
2. **Fence** — insert after the Review Enforcer Gate bypass line (pre-#500 ~L308; final L311), before `### Verification Gate`:
...
| Second-model P2 (2026-09-06): dispatch counter was `task`-tool-only; pi's `subagent` tool dispatches never counted — a docs-only micro change reviewed via a subagent dispatch would FALSE-BLOCK post-flip (micro relies on this counter as its only gate) | FIXED in PR #518 round 6: the counter now counts any sub-agent dispatch (`task` or `subagent` tool), consistent with the content-free F2 floor; the #285 isTaskSubAgent in-band exclusion is unchanged and still excludes sub-agents' own in-band dispatches; T1 gained a subagent-dispatch allow complement |
| Second-model P2 (2026-09-06): the "uniform ≥1-dispatch floor" prose describes the TIER dimension only; the #285 task-sub-agent session shape is a documented carve-out (parent-enforced dispatch) | Documented in 01-preflight.md Review Enforcer Gate section (blockquote) — an agent reading the docs knows the sub-agent exemption exists and that the merge-registry gate is the backstop; no behavior change |
