...
**Acceptance:** `--probe-jsonl FILE...` prints one JSON row per file `{path, last_timestamp, idle_proven, reason, age_h}` relative to `REAP_NOW_EPOCH`; **`idle_proven` iff age STRICTLY > `REAP_IDLE_HOURS` (an exactly-24h session survives — strict `>` matches the 0-idle->24h target and caps exposure at ~25h on the hourly cadence)**; giant single complete lines (≥150KB) parse; trailing partial/garbage line → previous parseable line; empty/whitespace/all-garbage/missing → `idle_proven:false`; ISO `.228Z` and `+00:00` micro timestamps both parse; numeric (float-epoch) top-level `timestamp` parses. Fixtures (below) green: `bash scripts/pi-reap-idle.test.sh`.
**Files:**
...
3. **Fixture pack A (parser; harness start):** giant complete line ≥150KB mid-file + at tail; trailing partial/garbage line; empty file; whitespace-only; all-garbage; missing file; `.228Z`; `+00:00` micro; **numeric float-epoch top-level timestamp**; fresh entry (age≈0 via `REAP_NOW_EPOCH`); threshold boundary via `REAP_NOW_EPOCH` (23:59:59 → not idle; **exactly 24:00:00 → not idle — strict `>`**; 24:00:01 → idle); `--probe-jsonl` JSON shape asserts. Run suite.
