...
- **E14** (L1608-1647): no format change; the round-trip gains explicit latch assertions — `formatReady`/`formatTurnStart` must NOT latch, `formatToolStart` MUST latch (the child-formatter → latch contract). Parse parity unchanged. ✓
...
|---|---|---|---|
| AC1 | A worked sub-agent (fresh markers + any parsed tool/message activity, incl. `tool_end`) is NEVER killed by `first-message-stall` — mid-turn quiet verdict OR frozen-age turn transition. **Carve-out (explicit):** a session with NO activity the PARENT can observe (no `tool_start`/`tool_end` marker, no tick reporting `tools>0`/`saw_msg`/`saw_tool` — incl. a sub-30s turn whose evidence is erased by the next `turn_start` before the first tick) remains subject to the M cut — indistinguishable from hung at the watchdog layer, #5926 preservation; evidenced-rare (0 of 5 recovered cuts). The latch's blind spot is parent-observability, not literal activity. | O; Indicator (1) regression boundary (worked + no-tools + quiet > M); Indicator (2) zero worked-session partials; Target's corrected semantics ("never cut on fresh markers + real activity") | E279a, E279a2, E279e; verification steps 3–4 |
...
- **Wedge-after-transition residual (Hardening 2):** a child that completes its transition markers (tool_end/turn_end/turn_start parsed → `streamAgeMs=0`, `toolAgeMaxMs=0`) and THEN stops emitting is still caught — under PRODUCTION defaults (T=30min → stateFresh window = max(2T, 2×interval) = 60min), `markerAge` accumulates into `effStreamAge` and stream-stall (clause 2, no turnActive requirement) fires at ~S=20min of marker silence, inside the fresh window; under the TEST harness (T=60s → 120s window) the same class falls to silence at T. Bounded in both (silence T / hard cap 2h / backstop 6.5h also fire) and arguably the correct semantics for a tools=0, turnActive=true, marker-stopped child (legacy E11/E12 class) — named here as the accepted trade; the AC3 "between-turn wedge preserved" claim refers to the tick-flowing variant (stream-stall at S of true quiet), not the marker-stopped-at-transition variant.
