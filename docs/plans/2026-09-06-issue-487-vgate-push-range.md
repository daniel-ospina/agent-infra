...
- **Second-model gate round 1 (P2s, all fixed in commit …)** — (1) reachability prose corrected: the dst=HEAD guard sits BEFORE src resolution, so `HEAD:HEAD`/`refs/heads/HEAD:HEAD` null AT THE GUARD (earlier plan wording said the src probe); the "tag literally named HEAD" clause was not code reality and now says the probe just fails on refs/heads/HEAD. (2) e2e scenario 58 pins the non-origin tier-B base preference (upstream/main base present, NO origin refs at all — an origin-fallback regression nulls the base → tier C staged block → RED). (3) e2e scenario 59 pins the multi-refspec union (block names BOTH ranges, not the staged WIP; allow once both verified) AND the any-tier-C-nulls-the-whole-command rule (`git push origin main nonexist` → whole-command staged block naming the WIP, where a per-refspec-union regression would allow via the already-verified content59m range). (4) audit-tier comment "first-A-wins" corrected to order-independent any-A-wins (code was always an OR boolean).

## Post-plan fold-in — #3716 (history-rewriting push, 2026-09-25)

- **Tier A's base selection is now CONDITIONAL (the plan's tier table above describes the fast-forward case).** The 2-dot `refs/remotes/<remote>/<dst>` range is used only when that ref is an ancestor-or-equal of the pushed tip. When it is NOT — the branch was rebased after being pushed, so the ref still points at the PRE-rebase tip on a DIFFERENT base — the 2-dot range is the whole base delta rather than the branch's change (measured in the field: 3 → 632, then the unconvergeable 629 on retry, tortoise #3716). Such a push is scoped **3-dot against the INTEGRATION base** (the first-push command), and the discard is reported separately as `gate_skip: non_fast_forward_push` (carrying `discardedCommits`) — a report, never an expansion of the verify set.
- **Fail-closed — the narrowing is taken only on TWO explicit proofs, and nothing else changes** (ordinals as in the code): (1) the integration base IS an ancestor-or-equal of the pushed tip — which makes `merge-base(base, src) == base`, so `git diff base...src` ≡ `git diff base src` and every path the range OMITS is byte-identical to the base TIP's content (the #3398 trust); and (2) the tracking ref is NOT an ancestor of the pushed tip (git exit 1, never a null). Proof (1) is load-bearing, not cosmetic: without it a branch that is merely BEHIND the base (or based on a sibling) narrows to `base...src`, which for a behind branch is EMPTY — reporting a content-REVERTING force-push as an up-to-date no-op. That was the first cut of this change; the verification dispatch found it, measured it (`2-dot = M base.txt`, `3-dot = empty`), and e2e scenario 85 now pins it. Any other probe outcome (including null) leaves the push on its pre-#3716 path: this change never silently re-bases a push it cannot prove.
- **What this does NOT change:** the argv builder (still exactly 2-dot for A, 3-dot for B — only the caller's base/tier choice moved), the first-push path, tier C, the whole-command rules, and the #755 subtraction (a rewrite push fails the push arm's guards 4/5 anyway, so the two mechanisms never overlap).
- **Pins:** e2e scenario 83 (a real rebase of an already-pushed branch: the block names the branch's own 3 files and NOT main's delta; verify-only-those-3 then pushes; its D row is ENOENT-skipped), scenario 84 (fast-forward control: the narrow incremental range is kept and NO rewrite report is emitted), scenario 85 (a branch BEHIND the base keeps the 2-dot scope — the fail-open guard; RED against the untightened first cut, green once proof (1) is present), and scenario 86 (a rewrite push whose narrowed range is EMPTY is reported as a rewrite, never as `push_range_empty` — an up-to-date no-op is a statement about content, not about the ref being replaced).
- **Stale-restatement note for the next reader:** the plan's tier-A sentences (and scenario 55 leg (c)'s 2-dot D-row claim) describe the pre-#3716 unconditional base. The argv facts stand; the BASE CHOICE no longer does.
