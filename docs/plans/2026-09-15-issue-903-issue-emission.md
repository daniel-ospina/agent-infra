...
1. `Closes #903` is **inert** — the issue is already closed. Note precisely: **two commits carry `Closes #903` as a trailer** — `d691294` and `f8100f9`; measured at `f580060`, `git log origin/main..f580060 --grep='^Closes #903$'` returns exactly those two. **Later commits on the branch quote the keyword mid-sentence** (`1fa56b1`, and the commit that records this sentence), which GitHub's parser counts as a live reference — as does this repo's own keyword scan (`scripts/record-review.sh`, which counts a mid-sentence ref for exactly that reason). So GitHub registers the PR as referencing #903, and merging would re-close it if it were ever re-opened. The inertness is a consequence of the issue already being closed, not of the PR abstaining. **No bare total is stated**: a commit that mentions the keyword adds one, so a total is invalidated by the very commit that writes it — the measurement is anchored to the revision it was taken at, and a trailer-only anchor (`^Closes #903$`) is the stable reading. The inertness is a consequence of the issue already being closed, not of the PR abstaining. The PR body states the situation rather than claiming abstention.
