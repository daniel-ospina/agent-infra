...
**Exactly one bundle-carrying position per arm, and six `null` positions:** `scopeFiles`:78, the `:3199` **`??` operand**, and the four internal fallbacks `:2386`/`:2391`/`:2419`/`:2425` (each switches the recorded side to the index — forwarding a mismatched ctx would subtract branch-authored content). At `:3199`: `resolvePushRangeScope(command, cwd, sub) ?? runStagedScope(cwd, null)` — the **first** argument carries the push arm's bundle (so the kill switch reaches the push leg), the **operand** does not.

**`runBranchScope` is a live arm:** `R = HEAD`, `B = HEAD`, `T` = the resolved trusted base (**not** the hardcoded `origin/main` it diffs). Pinned by a two-remote fixture (same-ref ⇒ inert; non-origin `T` ⇒ fires).
...
6. **Failure pins (test-only)** — no-throw; `makeGitSubCtx → null` ⇒ unchanged scope, no audit line; **isolating `pairMemo` pin: fail only the per-pair `diff T...B` with the per-arm diff and `ls-tree` succeeding, assert scope unchanged** (and the symmetric case) — without this an implementation caching an empty `Set` passes everything; kill-switch pin; `:2204` byte-identity and `:3199` source-string pins; per-arm handle requirements (commit arms need no `trackingRef`, so an unpushed branch still subtracts).
7. **E2E scenarios 77/78/79** in `index.e2e.test.ts` — merge-in-progress commit leg; push leg; kill switch.
...
- `cd extensions/verification-gate && npm ci --no-audit --no-fund` → `npx tsx index.test.ts` (296 pass) → `npx tsx index.e2e.test.ts` (84 pass) → `npx tsx subtract-scope.test.ts` (45 pass) → `node extensions/verification-gate/test-subtract-scope.mjs` (all checks pass).
- **Anti-vacuity, per arm** — assert `subtractions` is non-empty on every positive fixture (staged, worktree, wtPath, branch, gh-chain, tier-A push), not just that `files` is right. A `files`-only assertion cannot catch a no-op.
...
**Implementation status: complete.** 425 tests green (296 + 84 + 45). Three findings surfaced *during implementation* that review had not:
...
Fixture bugs found and fixed while writing scenarios 77/78/79 (each would have produced a misleadingly-passing test): after a merge, `git diff --cached` shows only the **incoming** side; and the branch must **diverge** from `T` or guard (3) correctly blocks the subtraction.
