...
**Trusted base:** `resolveTrustedBase(cwd)` → `{ref, oid}`: a `branch.<cur>.merge` that names a branch **different from the current branch**, or names a conventional integration branch (`main`/`master` — the `main` tracking `upstream/main` shape), resolved on `branch.<current>.remote` else `origin`; otherwise the **integration** remote's `refs/remotes/origin/main`; `null` when nothing resolves ⇒ no subtraction. Used by **both** legs so they cannot disagree. ⛔ A branch's own upstream ref is not a base EXCEPT when the branch itself is a conventional integration branch (`main`/`master` — the `main` tracking `upstream/main` shape): `git push -u` records the branch's own name in `branch.<cur>.merge`, and for any other name using it makes the push arm's guard (4) unsatisfiable (T equals the FIRST parent), silently disabling merge subtraction (#3398). ⛔ The base surface is not widened RELATIVE TO #755: the push command's remote is never consulted as a FALLBACK base (trusting a fork's `main` subtracted fork-only unverified content and silently ALLOWED the push — review F1, fail-open), and `refs/remotes/<remote>/HEAD` is not used (user-settable local state). A *declared* upstream naming another branch is still a base, exactly as before.
...
- **Vocabulary:** `SUBTRACT_SKIP_REASONS = ["base_identical_satisfied", "subtract_disabled_by_env"]` exported from `subtract-scope.ts`; `index.ts` declares `GATE_SKIP_REASONS` spreading it plus the 4 existing literals (`push_range_empty`, `delete_push_no_content`, `content_shape_exempt`, and #3716's `non_fast_forward_push`) and the 4 `#204` reasons — **10 distinct members, each literal written in exactly one place** (descriptive, never a pinned constant: `test-subtract-scope.mjs` asserts uniqueness, not the size).
...
**`runBranchScope` is a live arm:** `R = HEAD`, `B = HEAD`, `T` = the resolved trusted base (**not** the hardcoded `origin/main` it diffs). Pinned by a fixture with TWO remotes pointing at different commits, so which one is chosen is observable: an upstream naming a DIFFERENT branch (the configured `branch.<cur>.remote`) wins over the `origin/main` fallback. ⛔ An upstream naming the current branch is a base only when that name is `main`/`master` (the `main` tracking `upstream/main` shape — pinned by the `tracking remote WINS` unit test); for any other name it is the branch's own ref, which is not a base and resolves to the integration remote's `origin/main` (the #3398 fix).

**The push arm is only non-inert when `T` ≠ the destination's tracking ref.** Tier A's scope is `diff <tracking> <srcRef>` **while the tracking ref is an ancestor-or-equal of the pushed tip**; on the non-ancestor (history-rewriting) path the scope is `diff <trusted base>...<srcRef>` instead (§8, #3716), so for `git push origin main` (where `T` resolves to the same `refs/remotes/origin/main`) condition (1) is the exact *complement* of scope membership and nothing can ever be subtracted. The arm does its work on a **feature-branch push**, where `T` = the base branch and the tracking ref is the feature's own. Pinned by e2e scenario 80 (push leg) and scenario 81 (#3398: a merge of `origin/main` is de-flooded while the branch's own new file still blocks). On the rewrite path the guard set is not merely inert-by-complement: guard (5) cannot pass and every guard is required, so no subtraction occurs there either.
...
- `cd extensions/verification-gate && npm ci --no-audit --no-fund` → `npx tsx index.test.ts` (294 pass) → `npx tsx index.e2e.test.ts` (92 pass) → `npx tsx subtract-scope.test.ts` (50 pass) → `node extensions/verification-gate/test-subtract-scope.mjs` (all checks pass). (#3398 update — counts as of 2026-09-15.)
...
**Implementation status: complete.** 436 tests green (294 + 92 + 50). (#3398 update — counts as of 2026-09-15.) Findings surfaced *during* implementation and code review that the 12-cycle plan review had not:
...
**One more instance, and an instructive one.** The `branch` arm passed `"HEAD"` as both `bRef` and `srcRef`, so `makeGitSubCtx` resolved `HEAD` twice — a torn read letting condition (1) evaluate against one commit while (2)/(3) evaluated against another. Fixed by reusing the pinned B OID. The first pin written for it asserted `recordedSideOid() === b().oid`, which is **green against the bug** (with no concurrent writer both versions return the same OID) — caught by the same sabotage check. The property is only observable as a *count*, so the pin is now a PATH-shim subprocess counter: it asserts exactly one `rev-parse … HEAD` during the ctx build, and fails with `saw 2: ["rev-parse --verify --quiet HEAD", …]` against the two-probe version. Lesson sharpened: an equality assertion cannot pin a race; pin the *number of operations*, not their result.
...
> **Superseded in part (2026-09-25, tortoise #3716).** Two entries above no longer hold: the rebase push-leg de-flooding **landed** — the mechanism PROPOSED in agent-infra #737's 2026-09-13 comment (the same direction as #811's fix-direction option 1, which was folded INTO #737 on 2026-09-12): "make the tier conditional on ancestry" — tracking ref an ancestor ⇒ tier A 2-dot unchanged; otherwise 3-dot against the trusted base. ⛔ **Provenance correction (verification follow-up, 2026-09-25):** this is NOT the mechanism #737's `parked` disposition named. That disposition named a *named local-state pre-integration reference* (reflog `refs/heads/<branch>@{1}`, or an integration-time ref captured when the integration begins) with its own fail-closed and audit clauses, and it recorded that changing the mechanism is a design call. Taking the ancestry mechanism therefore **supersedes** the reflog one: the supersession is recorded on agent-infra #737 (comment, 2026-09-26: `issuecomment-5843159960`) and the reversal of tortoise #3716's own reframing is the reopen comment there. And "tier A/B/C semantics (unchanged)" therefore no longer describes the base SELECTION, only the argv builder. Nothing in the subtraction's guard set can subtract from a rewritten-history push's narrowed range — specifically guard (5) cannot pass (`tracking` not an ancestor of `srcRef` ⇒ not an ancestor of `srcRef^1`), guard (4) is independent and may pass, and every guard is required. **Pinned OIDs, not names:** the proof and the range diff read the SAME pinned commit OIDs (`resolveCommitOid`), and the narrowing base is the #3398 trusted base (`resolveTrustedBase`) — but only when it EQUALS the explicitly DECLARED integration ref (the per-clone `vgate.integrationRef` config, which is the activation surface; a checked-in `.vgate/integration-ref` is an agreement tripwire that must MATCH it when present — never an activation surface, because a shared, clone-relative name is inherited by fork clones where `origin` is the FORK; agent-infra #1491 — an undeclared repo gets no narrowing, and a base that is not the declaration — the checkout's own TOPIC upstream, a FORK's `main`, a legacy `main` — keeps the wider 2-dot scope: pinned by scenarios 89/90/91) — never the push remote's `main`, and never a ref name that a concurrent `fetch`/`update-ref` can retarget between the two reads (the tear `makeSubBundle` already forbids for T). The narrowing is taken only on TWO explicit proofs — the tracking ref is not an ancestor AND the integration base IS one (without the second, a branch merely BEHIND the base narrows to an EMPTY range and a content-reverting force-push is reported as an up-to-date no-op; the verification dispatch caught that in the first cut). e2e scenarios 83–89 carry the two-direction pin, that fail-open guard, the empty-range rewrite report, the trusted-base-vs-fork-main split (87), the deferred-audit pin (88), and the non-integration-base fail-open (89) — the scenario names keep #737's greppable trigger (`#737): rebase push range excludes base-identical paths`), and the skill's push-range paragraph (`skills/commit-workflow/workflow/01-preflight.md`) was updated in the same change.
