...
- Post-clamp current regime (Sep 1–5, whole fleet): daily total $0.08–$8.50; top single session $2.99; median $0.40; max context 296–406K → the 400K clamp is live, no ≥900K ceiling compactions in the new regime, per-marathon cacheRead bounded ≤ ~0.75B tokens (vs 1.3–4.1B pre-clamp).
...
Pre-clamp tortoise corpus (validated day shapes): cheap days Aug 10–12 = $0.50/$1.06/$2.18; spike days Aug 13 = $10.80, Aug 17 = $13.62; mid Aug 14/18/19 = $4.41–6.88. Post-clamp fleet Sep 1–5: $0.08–8.50/day; per-session median $0.40, max $2.99.

| Leg | Pre-registered trigger | Replay validation (would it have caught Aug 13 day-1?) | False-positive check (today's regime) |
|---|---|---|---|
| **D1 — fleet day-total delta** | day total > **3 × rolling-7-day mean** of prior day totals AND **≥ $5** | Aug 13: $10.80 vs baseline ~$1.25 = **8.6× → FIRES day 1** ✓. Aug 17: $13.62 vs ~$4.11 = 3.3× → FIRES day 1 of spike 2 ✓ | Sep 1: $8.50 vs 3.75 = 2.3× → no fire ✓ (busy #341/#372-style day, not runaway). Sep 5: $8.48 vs 3.63 = 2.3× → no fire ✓ |
| **D2 — per-session outlier** | any single session > **$5** (post-clamp regime; regenerate from corpus at impl) | Pre-clamp Aug 18 $5.94 / Aug 19 $4.41 sessions would fire — but D1 already fired Aug 17; D2 exists for the **quiet-day single-session runaway** (one 1M-window session at $8 on an otherwise $1 day) | Sep max $2.99 → no fire ✓ |
| **D3 — cache-read VOLUME (not share)** | fleet cacheRead tokens/day > **1.0B** on the 1M/qwen-ha class or pre-clamp-regime sessions | Aug 13 = 3.14B, Aug 17 = 4.09B → FIRES ✓; cheap days Aug 10–12 = 0.17–0.70B → silent ✓ | Post-clamp the 400K clamp bounds per-marathon cacheRead ≤ ~0.75B, so D3 is a **regression/qwen-ha-1M detector**, low frequency by design |
...
- **Escalation = dedup GitHub issue** (one OPEN `"fleet-cost-daily" in:title` → comment, else create) — the identical pattern `fleet-cost-weekly.sh` already ships. It has an **owner** (the same weekly-report reader; #341 PR-A's designated escalation recipient) and survives reboots (vs `/tmp/fleet-cost-weekly.log`, which is tmp-cleaned — the daily driver should log to `~/.pi/agent/state/`).
...
| `scripts/session-postmortem.sh` | **Parser unchanged** (sharing contract — the daily driver shells `--summary`). Optional future field: emit `models` seen (for D3 qwen-ha/1M-class routing) — defer unless corpus check at impl requires it | — | 0 (or +0.25 d if field added) |
