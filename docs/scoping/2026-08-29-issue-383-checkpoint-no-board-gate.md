...
2. **Scope — Option A (full converged scope):** ONE PR achieving the O/I/T:
   ...
   - Path rewrite: `extensions/sequence-enforcer/index.ts:819` hardcoded swarm path + 5 skill frontmatter comments → `$AGENT_INFRA_PATH/scripts` (env-overridable via existing `PARALLEL_CHECK_BIN`)
...
| Real path refs: index.ts:819 hardcoded `/Users/danielospina/swarm/operations/coordination/parallel_work_check.sh` + 5 comments (writing-plans:31, commit-workflow:17, executing-plans:15+41, issue-scoping:88/92) | grep |
