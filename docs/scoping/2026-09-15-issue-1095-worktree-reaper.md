...
`main-checkout` · `bare` · `self-checkout` · `worktree-locked` · `live-process` · `live-cwd` · `dirty` · `ignored-artifact` · `status-timeout` · `status-error` · `detached-unreachable` · `remote-only-ref` · `revocable-ref` · `open-pr` · `too-recent` · `unreadable-head` · `unreadable-path` · `unparseable-path` · `deferred` — plus `missing-dir`, which is **`remove` / `reason=prunable`** (a vanished checkout has nothing to lose; only the admin record is pruned — §3.4).
...
- **`unreadable-path`** — `canonicalize` returning `""` was read as "the directory is gone", so a checkout that merely could not be resolved (permissions, unmounted parent) was classified `prunable` and **deregistered while it was live**. `canonicalize` failing means *cannot resolve*, never *absent*: the two are now distinguished by an explicit existence test, and an existing-but-unresolvable path is preserved and its record protected from the end-of-pass `prune` (G9). **Cycle 8 found that existence test insufficient**: `[ -e ]`/`[ -L ]` are BOTH false when an *ancestor* is untraversable (permissions) or its mount is absent, so a **live** worktree under a `chmod 000` parent was still classified `prunable` and deregistered on `--apply` (`REMOVED=1`). Absence is now proven only by a **reachable parent that genuinely lacks the leaf**; an unresolvable parent, or a leaf that is still there, is `unreadable-path` (preserve + prune-blocked) (G9b).
...
Two more preserve paths were added in cycle 6 (both were `remove` paths that could lose committed work):

- **`remote-only-ref` / `revocable-ref`** — the containment arm of the reachable gate accepted **any** ref that contains the SHA. That claim is revocable in several routine ways, and the first fix (a `refs/remotes/*` blacklist) was **incomplete**: a blacklist cannot be completed. `git fetch --prune` drops a remote-tracking ref; `git fetch --prune --prune-tags` (or `fetch.pruneTags=true`) drops a **local-only tag** — verified on this host and pinned by A22's falsifier; `git stash drop` revokes `refs/stash`; `git bisect reset` revokes `refs/bisect/*`. Any of those leaves the commit gc-eligible with no record left. The survival test is therefore an **allowlist, not a blacklist**: only `refs/heads/*` — which `fetch` never writes and only an explicit `git branch -D` (never called here) removes — counts as containment survival. A holder outside it is `preserve`, with the ref **named** and the reason chosen by prefix (`remote-only-ref` for `refs/remotes/*`, the common case; `revocable-ref` otherwise) (A20, A22). The `merged` arm is untouched — it is ancestor-based and `MAIN_REF` is normally `refs/remotes/origin/main`, so `main` still fast-paths to `refs=merged` (A20c).

  Neither `remote-only-ref` nor `revocable-ref` blocks the prune **by reason**: for a present-directory record the commit is held by a ref, so deregistering the admin record cannot orphan it, and blocking on the reason let a single long-lived remote-only row disable the whole `prunable` reclaim path on every later pass — the "a reaper that silently never reaps" failure the header calls out. But a **gone-directory** record is different: its admin `HEAD` is a `git gc` reachability root, i.e. the commit's last **durable** handle, so deregistering it discards the only non-revocable reference and a later `fetch --prune` orphans the commit. The predicate is therefore **git's own `prunable` flag** — for the pass-start snapshot, exactly the set the global prune would then remove, and false for every present-directory record — not the reason string, **plus a bounded pre-prune re-read** (F17) because `git worktree prune` re-evaluates prunability at prune time and the snapshot cannot see a record that becomes prunable mid-pass. Pinned by A23 (present-dir row ⇒ prune still runs), A24 (gone-dir row ⇒ prune skipped, and the commit proven to survive `gc --prune=now`) and A25 (a checkout removed mid-pass ⇒ prune skipped, the commit survives; an unevaluable re-read ⇒ prune skipped, never "could not check, so reclaim anyway").
- **Removal-time re-check (not a preserve *reason*, a preserve *outcome*)** — the clean gate runs at classification and `git worktree remove` does **not** refuse ignored files, so a gitignored non-ephemeral file created in the window was deleted with the checkout while the row read `remove`. `remove_one()` now re-runs the clean gate before any deletion and again immediately before the removal fork; a late appearance is a **PRESERVE** (footer `LATE_PRESERVE=N`, exit `0`), not a removal and not a failure (A21a, A21b).
...
10. **The end-of-pass `git worktree prune` runs under `REAP_WT_REMOVE_TIMEOUT` too** (§9 cycle 4). It is the *actual* removal for every `prunable` row and it runs after the classification loop — i.e. beyond the global budget's reach — so it needs its own watchdog; a hang there would wedge a pass that had already finished classifying and hold the lock until it went stale 30 min later. A timeout is `PRUNE-FAIL` + `FAILED=n` + `exit 4`, with the records left registered.
11. **The three per-record PROPERTY probes are watchdosed as well** (`merge-base`, `for-each-ref --contains`, `log -1`, under `REAP_WT_GIT_TIMEOUT`, default 20 s), plus the pass-level repository-resolution probes (one `rev-parse --git-common-dir`, and main-ref detection's `symbolic-ref`/`rev-parse`). This is a correctness requirement, not polish: the global budget is only checked **between** records, so an unwatched probe hanging *inside* one record hangs the whole pass — the failure mode this tool exists to prevent. Each takes the **preserving** arm on a timeout (`merged=0`, cannot-establish ⇒ `""` ⇒ PRESERVE, `unreadable-head`), so a hung probe can never be read as a pass. Found in §9 cycle 4.
...
`git worktree prune` has **no path filter**, so it cannot be aimed at the records we decided to reclaim: a global prune would also deregister any *preserved* record that git happens to consider prunable. The prune therefore runs **once, at the end of the pass, and only after checking that no preserved record could be prunable** — the predicate is **git's own `prunable` flag** on the record (the exact set the prune would remove), plus belt-and-braces blocks for a record preserved because a probe could not be evaluated (`unreadable-path`, `status-error`, `status-timeout`, `unparseable-path`, `worktree-locked`) or because its commit is on **no** ref at all (`detached-unreachable`, where losing the admin `HEAD` is unrecoverable). When the prune is skipped, the skip is stated on stdout and logged, and the reclaimable records simply wait for a later pass (`PRUNE_BLOCKED` in the footer). Idempotence is unaffected either way.
...
**The clean gate is re-run on the removal path (cycle 6).** Classification is not the last word: `git worktree remove` does not refuse ignored files, so the ignored half of the clean gate exists precisely for a class git will delete silently. `remove_one()` therefore re-runs `clean_gate` **before any deletion** (so a checkout that changed since classification does not even lose its ephemerals) and **again immediately before** `git worktree remove`. Both probes are `status` calls under `REAP_WT_STATUS_TIMEOUT`, so the re-check cannot become the unbounded fork it guards against. A late appearance returns the internal code `2` ⇒ PRESERVE (`LATE_PRESERVE=N` in the footer, process exit `0`) — deliberately **not** exit `4`, which is the machine-readable "a removal failed"; the banner prints the re-check's actual verdict (`dirty` / `ignored-artifact` / `status-timeout`), because "the checkout changed" is the wrong report for a probe that could not be evaluated (A21d). This **narrows** the window to the fork itself; it does not close it (that would need a lock on the checkout), which is why the gate is re-run rather than the classification trusted.
...
| The end-of-pass `git worktree prune` blocks | `PRUNE-FAIL` + `FAILED=1` + `exit 4`; the gone-checkout record stays registered | `A17` |
| A branch ref was force-deleted but `worktree list` still prints it | NOT reported as `branch-retained`; PRESERVE + record not pruned (dir present and gone) | `A18a`, `A18b` |
| The origin-remote slug probe fails (open-PR gate unverifiable) | `exit 3` with nothing removed — never read as "no GitHub remote" (which silently disabled the veto) | `A19`, `A19b` |
| A detached HEAD held ONLY by `refs/remotes/*` | Preserved with `remote-only-ref`; the revocable ref named; the commit proven to reach *no* ref after `fetch --prune` | `A20` |
| A detached HEAD held ONLY by a local tag | Preserved with `revocable-ref`; the tag named; `fetch.pruneTags=true` proven to delete the tag and take the commit to no ref | `A22` |
| A commit held only by a remote-tracking `MAIN_REF` | Still `refs=merged` — the merged fast path is not disqualified by the revocable-ref rejection | `A20c` |
| A present-dir `revocable-ref` row + a gone-dir `prunable` row | The prune still runs; the prunable record is deregistered and the preserved one stays registered | `A23` |
| A gone-dir `revocable-ref` row (its admin `HEAD` is the last durable handle) | The prune is skipped (`PRUNE_BLOCKED=1`); the record stays registered and the commit survives `fetch --prune` + `gc --prune=now` | `A24` |
| A live worktree whose PARENT is untraversable | Preserved with `unreadable-path` — never `prunable`; the admin record is not deregistered | `G9b` |
| A gitignored file appears after classification | Preserved (`LATE_PRESERVE=1`, exit `0`); the file survives; the ephemeral pre-delete had not run | `A21a` |
| A gitignored file appears after the ephemeral pre-delete | Preserved by the second re-check; the file survives and the pre-delete provably ran | `A21b` |
| The same fixture with no late write | Still reclaimed — the re-check is not a veto | `A21c` |
| The removal-time re-check cannot be evaluated (`status` times out) | Preserved, and the banner names `status-timeout` — never reported as "the checkout changed" | `A21d` |

## 6. Wiring check

- `scripts/pi-reap-worktrees.sh` (new) — the reaper.
- `scripts/pi-reap-worktrees.test.sh` (new) — hermetic suite (**216 assertions, all green**; see §9 for the count's history); registered in `.github/workflows/ci-main.yml` alongside `pi-reap-idle.test.sh` (post-merge main-push family — `ci.yml` has no per-PR shell-script block). `ci-main.yml` is a **content-locked** workflow, so `scripts/workflow-lock.json` is re-hashed in the same change (`node scripts/check-workflow-lock.mjs --update-lock`); the lock gate fails closed otherwise, which is exactly how this omission was caught by `ci / unit-test` on the PR rather than merged.
...
### Cycle 4 — code review at PR time (fresh context), and its findings

A fresh-context reviewer examined the PR diff against the **declared adversarial threat surface** (§2.2 / the issue's scoping comment). Verdict: **no bypass of clean / reachable / unreferenced / aged was reproduced; no path was found where a failed or hung removal is reported as `REMOVED`; the deferred-record-vs-global-prune leak holds.** Four P2 findings, none of which destroys or orphans committed work on its own. Disposition:

- **F1 (boundedness claim, fixed).** Two real holes in "no unbounded subprocess": (a) the **end-of-pass `git worktree prune`** — the actual removal for every `prunable` row, running after the loop and therefore outside the budget — was a bare call; (b) the three **per-record property probes** (`merge-base`, `for-each-ref --contains`, `log -1`) were unwatched, and since the budget is only checked between records, a hang inside one record hung the whole pass. Both fixed: `prune` under `REAP_WT_REMOVE_TIMEOUT`, the property probes (and the pass-level repo-resolution probes) under the new `REAP_WT_GIT_TIMEOUT`, each taking the preserving arm on timeout. Pinned by `A17` (falsifier: pre-fix, a 3 s prune with a slow-`prune` shim exits **0** with `REMOVED=1` and the record gone; with the fix it exits **4** with `FAILED=1` and the record registered).
- **F2 (false survival mechanism, fixed).** `git worktree list --porcelain` keeps emitting `branch refs/heads/X` — with `HEAD 0000…0` — after X is force-deleted, so `commits_survive()` reported `refs=branch-retained:X` for a ref that does not exist. Verified on this host. On the directory-present path the `dirty` gate catches it first (an unborn HEAD makes the index look fully staged), but on the **gone-directory** path the clean and aged gates are skipped by design, so the false label reached `remove/reason=prunable` and the prune deregistered the admin record — the last handle on that HEAD, including `worktrees/<id>/logs/HEAD`. Now the ref is **resolved** before it is reported; a deleted ref falls through to the containment check, which for `0000…0` fails (`for-each-ref` exits 129) ⇒ `detached-unreachable` + `PRUNE_BLOCKED`. Pinned by `A18a`/`A18b` (falsifier: pre-fix, the same gone-dir fixture exits **0** with `REMOVED=1` and the record pruned).
- **F3 (remote-tracking refs treated as permanent survival — ACCEPTED as a documented residual).** A detached HEAD held only by `refs/remotes/*` is reported `reachable-from:<ref>` and the checkout is deleted; a later `git fetch --prune` can drop that ref and make the commit gc-eligible. Accepted rather than changed: excluding remote refs would also disqualify the merged path (main is typically a remote ref) and would over-preserve the `/private/tmp` scratch class the issue wants reclaimed; the mitigation is that the holding ref is always **named in the output**, so the operator can judge it. Recorded here and in §2.2.
- **F4 (removal-time TOCTOU for a newly created ignored file — ACCEPTED as declared out-of-scope).** The clean gate runs at classification; `git worktree remove` is the second gate but does *not* refuse ignored files, so an ignored non-ephemeral file created in the window is deleted with the checkout. This is inherent without holding a lock on the checkout, and the window is bounded by the removal watchdog. It belongs to the declared out-of-scope class (races against concurrent mutation of the checkout), now stated explicitly rather than left implicit.

**Assertion count:** 129 → 142 (the thirteen new `A17`/`A18a`/`A18b` assertions).

### Cycle 5 — second review cycle of the adversarial bound, and its finding

The adversarial loop's bound is **2 cycles** (the declared threat surface, not reviewer exhaustion). The second cycle ran against the fix delta above and reproduced **one new in-scope bypass**, so it did **not** return `THREAT SURFACE COVERED`:

- **F5 (fail-open on the origin-slug probe — FIXED).** `pr_head_refs_load()` ran `git remote get-url origin` and **discarded its exit status**, reading an empty URL as "no GitHub remote". That sets `PR_APPLICABLE=0`, which makes `classify_one()` skip the open-PR veto *entirely* — a gate defeated by making its probe unverifiable, i.e. the same class as reading an empty `ps` table as "no live processes", and the same class `G5c` already pins for `gh` itself. Reproduced deterministically: a repo WITH a GitHub origin, a worktree on an aged clean branch that `gh` reports as an open PR head, and a git shim that fails only `remote get-url` → pre-fix output `GH=na`, `REMOVED=1`, worktree **removed**. `git remote get-url origin` exits 2 when origin genuinely does not exist and 0 with a URL otherwise, so only those two are decidable; the probe is now watchdosed and anything else (including a timeout) is `exit 3` with nothing removed. Pinned by `A19` (broken probe ⇒ `exit 3`, `GH=unavailable`, candidate not classified reclaimable, armed pass removes nothing, `GH slug probe failed` logged) and `A19b` (a genuinely absent origin is still N/A ⇒ `exit 0` and still reclaims). Falsifier: pre-fix, the same fixture exits **0** with `REMOVED=1` and the open-PR worktree gone.

**Bounded-exit disclosure:** the loop spent its bound of 2 cycles. Cycle 2's finding is fixed and pinned by a test **and** a falsifier, but **no third reviewer confirmed the fix** — the accepted residual is exactly that, and it is stated here rather than presented as an unbounded clean exit.

**Assertion count:** 129 → 142 (cycle 4: `A17`/`A18a`/`A18b`) → 151 (cycle 5: `A19`/`A19b`).

### Cycle 6 — closing the two accepted residuals F3 and F4 (closed-out, post-bound)

Cycle 4/5 filed F3 and F4 as accepted residuals (issues #1104, #1105) behind the `[ADVERSARIAL-BOUND] cycles=2` disclosure. Both were then **closed out** rather than left as documented limitations, because each is a `remove` path that can lose committed work — the failure mode the gate exists to prevent — and a documented limitation is only acceptable when the exposure is genuinely intrinsic:

- **F3 → fixed (`remote-only-ref` / #1104).** Option 1 from the follow-up issue: containment must be in a ref `git fetch --prune` cannot revoke. The "exclude remote refs disqualifies the merged path" objection is answered by the **order** of the arms — `merge-base --is-ancestor` runs first and is untouched — and pinned by `A20c` (a commit held only by `refs/remotes/origin/main` is still `refs=merged`). The `/private/tmp` scratch class stays reclaimable unless its commit was **only ever pushed**; the over-preservation is the safe direction and is *visible* — the revocable ref is named in the row. Cost accepted: a detached scratch worktree whose commit survives only on a remote-tracking ref is now preserved rather than reclaimed. Pinned by `A20`; falsifier run for real (pre-fix suite: `A20` fails with `reason=reclaimable` on a remote-tracking holder, and `--apply` deletes the checkout). **Cycle 7 found this shape incomplete — a `refs/remotes/*` blacklist still admits `refs/tags/*`; see below.**
- **F4 → narrowed (`remove_one` re-checks / #1105).** The issue's "cheapest real improvement": re-run the ignored half of the clean gate immediately before removal and refuse if anything non-ephemeral appeared. Implemented twice over — before **any** deletion and again immediately before the removal fork — because the ephemeral pre-delete can itself run for up to `REAP_WT_REMOVE_TIMEOUT`. A late appearance is a PRESERVE returned as internal code `2` (exit `0`, `LATE_PRESERVE=N`), never exit `4`. **What is still not closed, stated plainly:** a file written in the gap between the final `status` probe and the `git worktree remove` fork is still deleted. That gap is the fork itself, and closing it needs a lock on the checkout — which is why this remains a *narrowed* window and not a closed one. Pinned by `A21a`/`A21b`/`A21c`; falsifier run for real (pre-fix suite: the late `.env` is destroyed in both windows while the pass reports a clean `REMOVED`).

**Assertion count:** 151 → 180 (cycle 6: `A20`/`A20c`/`A21a`/`A21b`/`A21c`). Falsifier evidence: the new suite run against the pre-fix script is `166 passed, 14 failed` — all 14 failures confined to `A20`/`A21`, none elsewhere.

### Cycle 7 — fresh re-review of the F3/F4 close-out, and its three findings

A fresh-context adversarial reviewer examined head `79672fd` against the same declared threat surface. **Verdict: not clean** — it returned three ISSUE blocks rather than `THREAT SURFACE COVERED`. All three are fixed in the same close-out (this is a new review round on a new head; the PR's `[ADVERSARIAL-BOUND]` disclosure is updated accordingly and is not presented as an unbounded clean exit):

- **F6 (bug, P1, confidence 85) — the F3 fix was an incomplete blacklist; a LOCAL TAG re-opened the class. FIXED.** A detached HEAD held only by `refs/tags/ops/scratch` was classified `remove / refs=reachable-from:refs/tags/ops/scratch` and the checkout deleted; `git fetch --prune --prune-tags` (or `fetch.pruneTags=true`) then deletes a local-only tag, leaving the commit on no ref — the exact F3 exposure, re-opened for a different ref class. The reviewer also noted a blacklist cannot be completed: `git stash drop` revokes `refs/stash`, `git bisect reset` revokes `refs/bisect/*`. **Fix:** the containment test is now an **allowlist** — only `refs/heads/*` counts as survival (the merged arm is untouched); any other holder is `preserve` with the ref named, reason `remote-only-ref` for `refs/remotes/*` and the new `revocable-ref` otherwise. Pinned by `A22`; the `pruneTags` half was **verified on this host** and is asserted by A22's own falsifier (`fetch.pruneTags=true` deletes the tag; the commit is then on no ref).
- **F7 (bug, P2, confidence 80) — `remote-only-ref` set `PRUNE_BLOCKED` for a present-directory record. FIXED.** A live-dir record is not prunable, so the flag was spurious and disabled the end-of-pass prune **globally** — with one long-lived remote-only row present, every `prunable` record stayed registered on every later pass: the "a reaper that silently never reaps" failure the header itself calls out. **Fix:** neither `remote-only-ref` nor `revocable-ref` blocks the prune — the commit is held by a **ref**, so deregistering the admin record cannot orphan it (that argument only holds for `detached-unreachable`, where the record's `HEAD`/reflog is the last handle). Pinned by `A23` (present-dir revocable-ref row + gone-dir prunable row ⇒ `PRUNE_BLOCKED=0`, the prune runs, the prunable record is deregistered, the preserved one stays).
- **F8 (bug, P2, confidence 65) — the late-preserve banner misreported an unevaluable probe as a checkout change. FIXED.** All non-clean removal-time re-checks mapped to one stdout line, `the checkout changed between classification and removal` — wrong for `status-timeout`/`status-error`, where the probe could not be evaluated at all, and those need a different operator response. Counters and exit code were already correct (code `2` never leaked into `REMOVED`/`FAILED`/exit `4`). **Fix:** `remove_one()` leaves the verdict in `REMOVE_SKIP_VERDICT` and the banner prints it — `PRESERVED — removal-time clean re-check not clean (<verdict>)`. Pinned by `A21d`.

**Residual, declared out of scope (recorded, not chased):** a record whose admin `gitdir` file is missing is omitted entirely by `git worktree list`, so it is never in `WT_LIST` and cannot set `PRUNE_BLOCKED`, yet the end-of-pass `git worktree prune` deletes its admin dir — the last holder of a detached HEAD. Reproducible only by tampering with git's admin files, so it falls under the declared out-of-scope class *"a hostile local user who can rewrite the script or the repo itself"*. Stated here rather than silently dropped.

**Attempted and not bypassed by the reviewer:** the `--count=64` containment bound (sound toward preservation once the durability test is an allowlist), the merged fast path (A20c holds), post-classification file destruction (both re-checks fire; the residual fork-gap is disclosed), code-`2` leakage into counters or exit codes (none), and the re-check acting as a veto (A21c reclaims).

**Assertion count:** 180 → 201 (cycle 7: `A21d`/`A22`/`A23`).

### Cycle 8 — second fresh re-review of the close-out, and its findings

A second fresh-context adversarial reviewer examined head `57910b3`. **Verdict: not clean** — four ISSUE blocks (one P0, one P1, two test-gaps), not `THREAT SURFACE COVERED`. **Two of the four were fixed**; the other two were the test-gaps for those two fixes, so all four are closed here. What the reviewer *did* endorse is recorded below too, because a `NOT clean` verdict that also lists what held is more useful than the verdict alone.

- **F9 (bug, P0, confidence 95) — the F7 change over-generalized, and reopening the prune path for a GONE-dir revocable-ref record re-opened the orphan class. FIXED.** Dropping `PRUNE_BLOCKED` for `remote-only-ref`/`revocable-ref` is correct for a **present**-directory record (git cannot prune it) but wrong for a **gone** one: the admin `HEAD` is a `git gc` reachability root, i.e. the commit's last **durable** handle, so a global path-filterless prune deregisters the preserved record — while the classification's own message says *"pruning the record would ORPHAN it"* — and a later `fetch --prune` leaves the commit on no ref. Reproduced end-to-end by the reviewer. **Fix:** the prune-block predicate is now **git's own `prunable` flag** (the exact set the prune would remove, false for every present-dir record), plus the existing belt-and-braces reason blocks. Pinned by `A24`, which also asserts the durable-handle claim itself: after the remote branch is deleted and the tracking ref pruned, the commit still resolves after `git gc --prune=now`. Falsifier run against `57910b3`: `A24` fails with `PRUNE_BLOCKED=1` missing, `PRUNED` logged, and the record deregistered.
- **F10 (bug, P1, confidence 90) — a failed LEAF stat is not proof of absence; the cycle-2 `unreadable-path` fix was incomplete. FIXED.** With an untraversable **ancestor** (`chmod 000 parent`) or an absent mount, `[ -e ]` and `[ -L ]` are both false for a checkout still on disk — the exact "cannot resolve" state the code maps to `unreadable-path` — so a **live** worktree was classified `remove / reason=prunable` and its admin record deregistered on `--apply` (`REMOVED=1`, exit 0), contradicting the header and §2.2's cycle-2 P0 claim. **Fix:** absence is proven only by a **reachable parent that genuinely lacks the leaf**; an unresolvable parent (or a leaf that is still there) is `unreadable-path` → preserve + prune-blocked. Pinned by `G9b`. Falsifier: pre-fix, the same fixture classifies `reason=prunable` and `--apply` deregisters the live worktree.
- **F11 (test-gap, P2, confidence 90) — `G9` only `chmod 000`s the LEAF, which is exactly the case where `[ -e ]` still succeeds, so the `unreadable-path` class was never tested for the ancestor variant. CLOSED** by `G9b`.
- **F12 (test-gap, P2, confidence 95) — no fixture exercised the GONE-dir revocable-ref branch, where `PRUNE_BLOCKED` is load-bearing; `A23` as written gave false confidence because its revocable-ref record has a live directory and therefore cannot be pruned anyway. CLOSED** by `A24`.

**Attempted and NOT bypassed by this reviewer** (recorded because a bounded exit must state what held): the `--count=64` containment bound (it sorts `refs/heads/*` first, so it can only over-preserve); internal code `2` leaking into `REMOVED`/`FAILED`/exit (`LATE_PRESERVE` + exit 0 only); the post-classification file race between the final `clean_gate` and the `git worktree remove` fork (accurately disclosed in the header and §3.6).
**Residuals the reviewer judged OUT of the declared surface** (config rewrite / admin-file tampering, i.e. the declared *"hostile local user who rewrites the script or the repo itself"* class — recorded, not chased): a commit held only by a revocable `refs/remotes/*` **MAIN_REF** (A20c; deliberate), a fetch refspec that targets `refs/heads/*` (`+refs/heads/feat/*:refs/heads/feat/*`, under which `fetch --prune` deletes a local branch and falsifies the header's "fetch never writes refs/heads" wording), and a record whose admin `gitdir` file is missing (git omits it from `worktree list`, so it cannot block the prune).

**Assertion count:** 201 → 216 (cycle 8: `G9b`, `A24` + the A24 durable-handle leg). Falsifier evidence: the new suite run against `57910b3` is `207 passed, 9 failed` — all 9 failures inside `G9b`/`A24`, none elsewhere.

**Bound status (stated plainly):** the PR's own adversarial bound was spent long before this close-out; cycles 7 and 8 were additional fresh rounds run at the instruction of the change owner. Cycle 8's four findings are fixed and falsified, but **no further fresh reviewer confirmed that fix** — so this remains a bounded, disclosed exit and **not** a clean completion.

### Cycle 9 — third fresh re-review: one P2 test-gap, F9/F10 confirmed

A third fresh-context adversarial reviewer examined head `bfc57f8`. **Verdict: not clean** (one ISSUE block, not `THREAT SURFACE COVERED`) — but it **confirmed F9 and F10 resolved** and **reproduced no in-scope bypass**, and its single finding was a test-quality gap, not a defect in the tool:

- **F13 (test-gap, P2, confidence 95) — A24's `gc` leg was an INERT assertion. FIXED.** The leg deleted the revocable ref, pruned the tracking ref, then ran `git gc --prune=now` and expected the commit to survive *because* the worktree admin `HEAD` holds it. But the MAIN repo's own reflog also held the commit, and `git gc` keeps reflog-reachable objects — so it survived whether or not the admin record existed. The assertion could not fail, so it proved nothing. **Fix:** `git reflog expire --expire=now --expire-unreachable=now --all` before the `gc`, leaving the admin `HEAD` as the only handle. The leg now discriminates: it passes at `bfc57f8` and fails at `57910b3` (*"the commit was collected"*).
- **F14 (test-gap, reported unfiled by the reviewer, same class) — A15 was mutation-blind. CLOSED.** The apply leg fires BOTH `PRUNE_BLOCKED` sites (classify-deferral and remove-deferral), so deleting either one alone left the footer at `PRUNE_BLOCKED=1` and the assertion noticed nothing. A DRY-RUN leg was added, which reaches only the classify-deferral site; deleting only that line now turns exactly that assertion red (mutation-verified: `217 passed, 1 failed`).

Falsifier evidence: the revised suite against `57910b3` is `208 passed, 10 failed`, all 10 inside `G9b`/`A24` (the A24 `gc` leg flipped from inert pass to genuine failure).

### Cycle 10 — fourth fresh re-review: one in-scope P1, F17

A fourth fresh-context adversarial reviewer examined head `22754d9`. **Verdict: not clean** — it confirmed F13/F14 fixed and falsifier-verified, and then **reproduced one in-scope bypass**:

- **F17 (bug, P1, confidence 85) — the prune-block predicate read the `prunable` flag from the PASS-START list snapshot, but `git worktree prune` RE-EVALUATES prunability at prune time. FIXED.** A record that was present at list time and gone by prune time carries no flag in the snapshot, so it never set `PRUNE_BLOCKED` — and the path-filterless prune sweeps its admin record. Where that record was preserved as `remote-only-ref`/`revocable-ref`, the admin `HEAD` is the commit's **last durable handle**, so the prune orphans the commit: the F9 class, through a window the snapshot cannot see (threat classes 2 and 4). No hostile actor is required — any external checkout removal mid-pass opens it. The reviewer reproduced it 9/9 with a control (the same fixture without the gone-dir prune-arming row survives `gc`). **Fix:** re-list (`wt_list_load`, bounded) immediately before the prune and block unless every now-prunable path ALREADY carried the flag at classification (the intended reclaim targets); an unevaluable re-read, or an unusable scratch file, is **fail-closed** (block). Pinned by `A25`, which removes the live checkout mid-pass and asserts the prune is skipped, the record is still registered, and the commit survives `gc --prune=now` after every other handle is expired; plus the fail-closed leg (a `list` that fails only after classification probes ⇒ `PRUNE-RECHECK-FAILED`, prune skipped). Falsifier run against `22754d9`: `225 passed, 8 failed`, all 8 inside `A25` — including *"the commit was collected: the global prune orphaned it"*.

**Assertion count:** 216 → 218 (cycle 9) → 233 (cycle 10: `A25` +15). Falsifier evidence: the revised suite against `22754d9` is `225 passed, 8 failed`, all 8 inside `A25`.
