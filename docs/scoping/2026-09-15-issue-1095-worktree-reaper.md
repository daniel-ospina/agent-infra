...
`main-checkout` · `bare` · `self-checkout` · `worktree-locked` · `live-process` · `live-cwd` · `dirty` · `ignored-artifact` · `status-timeout` · `status-error` · `detached-unreachable` · `remote-only-ref` · `open-pr` · `too-recent` · `unreadable-head` · `unreadable-path` · `unparseable-path` · `deferred` — plus `missing-dir`, which is **`remove` / `reason=prunable`** (a vanished checkout has nothing to lose; only the admin record is pruned — §3.4).
...
- **`remote-only-ref`** — the containment arm of the reachable gate accepted **any** ref, including `refs/remotes/*`. A later `git fetch --prune` (routine after a squash-merge with branch auto-delete) drops that ref, so the "commits survive" claim was **revocable** and the commit became gc-eligible with no record left. Only a ref `fetch --prune` cannot touch now counts as survival; a remote-tracking-only holder is `preserve` with the revocable ref **named** (A20). The `merged` arm is untouched — it is ancestor-based and `MAIN_REF` is normally `refs/remotes/origin/main`, so `main` still fast-paths to `refs=merged` (A20c).
...
| A commit held only by a remote-tracking `MAIN_REF` | Still `refs=merged` — the merged fast path is not disqualified by the remote-ref rejection | `A20c` |
