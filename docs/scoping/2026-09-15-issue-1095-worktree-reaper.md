...
- **`remote-only-ref` / `revocable-ref`** — the containment arm of the reachable gate accepted **any** ref that contains the SHA. That claim is revocable in several routine ways, and the first fix (a `refs/remotes/*` blacklist) was **incomplete**: a blacklist cannot be completed. `git fetch --prune` drops a remote-tracking ref; `git fetch --prune --prune-tags` (or `fetch.pruneTags=true`) drops a **local-only tag** — verified on this host and pinned by A22's falsifier; `git stash drop` revokes `refs/stash`; `git bisect reset` revokes `refs/bisect/*`. Any of those leaves the commit gc-eligible with no record left. The survival test is therefore an **allowlist, not a blacklist**: only `refs/heads/*` — which `fetch` never writes and only an explicit `git branch -D` (never called here) removes — counts as containment survival. A holder outside it is `preserve`, with the ref **named** and the reason chosen by prefix (`remote-only-ref` for `refs/remotes/*`, the common case; `revocable-ref` otherwise) (A20, A22). The `merged` arm is untouched — it is ancestor-based and `MAIN_REF` is normally `refs/remotes/origin/main`, so `main` still fast-paths to `refs=merged` (A20c).
  ...
  Neither `remote-only-ref` nor `revocable-ref` sets `PRUNE_BLOCKED`: the commit is held by a **ref**, so deregistering the admin record cannot orphan it — that is precisely the `detached-unreachable` case, where the record's `HEAD`/reflog *is* the last handle. Blocking on them (as the first fix did) let a single long-lived remote-only row disable the whole `prunable` reclaim path on every later pass, which is the "a reaper that silently never reaps" failure the header calls out. Pinned by A23.
